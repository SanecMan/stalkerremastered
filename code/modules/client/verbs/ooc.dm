...
/client/verb/fix_chat()
	...
	if (!chatOutput || !istype(chatOutput))
		var/action = alert(src, "Не нашел выходные данные._acrccos\nПересоздать?", "кодер или пидор", "давай", "не")
		if (action != "давай")
			return
		chatOutput = new /datum/chatOutput(src)
		chatOutput.start()
		action = alert(src, "Релоадаю гунчат", "", "починилось", "не")
		if (action == "починилось")
			log_game("GOONCHAT: [key_name(src)] Had to fix their goonchat by re-creating the chatOutput datum")
		else
			chatOutput.load()
			action = alert(src, "А вот теперь? Чат немного будет мерцать и загружаться", "", "починил", "не")
			if (action == "починил")
				log_game("GOONCHAT: [key_name(src)] Had to fix their goonchat by re-creating the chatOutput datum and forcing a load()")
			else
				action = alert(src, "Ну у меня кончились идеи.\nМогу загрузить не гуновский чат", "", "спасибо за помощь", "включи не гуновский чат")
				if (action == "включи не гуновский чат")
