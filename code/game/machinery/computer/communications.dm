	...
	switch(href_list["operation"])
		...
		if("nukerequest") //When there's no other way
			if(authenticated==2)
				...
				add_lenta_message(null, "0", "Nuclear Self Destruct Codes Requested", "NanoTrasen", "The codes for the on-station nuclear self-destruct have been requested by [usr]. Confirmation or denial of this request will be sent shortly.")
				CM.lastTimeUsed = world.time
