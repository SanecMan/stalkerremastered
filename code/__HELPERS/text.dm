...
/proc/sanitize(t,list/repl_chars = null)
	return rrhtml_encode(sanitize_simple(t,repl_chars))
...
/proc/adminscrub(t,limit=MAX_MESSAGE_LEN)
	return copytext((rrhtml_encode(strip_html_simple(t))),1,limit)
...
/proc/stripped_input(mob/user, message = "", title = "", default = "", max_length=MAX_MESSAGE_LEN, no_trim=FALSE)
	...
	if(no_trim)
		return copytext(rrhtml_encode(name), 1, max_length)
	else
		return trim(rrhtml_encode(name), max_length) //trim is "outside" because rhtml_encode can expand single symbols into multiple symbols (such as turning < into &lt;)
...
/proc/stripped_multiline_input(mob/user, message = "", title = "", default = "", max_length=MAX_MESSAGE_LEN, no_trim=FALSE)
	...
	if(no_trim)
		...
		return trim(rrhtml_encode(name), max_length)
